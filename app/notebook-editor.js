...
export default function NotebookEditorScreen() {
    ...
    PanResponder.create({
      ...
      onPanResponderRelease: (_, g) => {
        ...
        if (g.dx < -threshold && currentPageIndex < pages.length - 1) {
          ...
          Animated.timing(swipeAnim, { toValue: -SCREEN_W, duration: 200, useNativeDriver: Platform.OS !== 'web' }).start(() => {
          ...
          Animated.spring(swipeAnim, { toValue: 0, tension: 120, friction: 14, useNativeDriver: Platform.OS !== 'web' }).start();
        }
      },
    })
  ).current;
  ...
  const showSaveIndicator = useCallback((status) => {
    ...
    if (status === 'saved') {
      setTimeout(() => {
        Animated.timing(saveIndicatorAnim, { toValue: 0, duration: 500, useNativeDriver: Platform.OS !== 'web' }).start();
      }, 1500);
    }
  }, []);
