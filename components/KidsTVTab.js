...
function BounceIn({ children, delay = 0 }) {
  ...
  useEffect(() => {
    Animated.parallel([
      Animated.spring(scale, { toValue: 1, delay, tension: 80, friction: 8, useNativeDriver: false }),
      Animated.timing(opacity, { toValue: 1, delay, duration: 250, useNativeDriver: false }),
    ]).start();
  }, []);
