...
export function HelpModal({ visible, onClose }) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      Animated.parallel([
        ...
        Animated.spring(slideAnim, { toValue: 0, tension: 50, friction: 12, useNativeDriver: Platform.OS !== 'web' }),
        Animated.spring(scaleAnim, { toValue: 1, tension: 50, friction: 12, useNativeDriver: Platform.OS !== 'web' }),
      ]).start();
    } else {
      fadeAnim.setValue(0);
  ...
  const handleClose = useCallback(() => {
    Animated.parallel([
      ...
      Animated.timing(slideAnim, { toValue: 40, duration: 220, useNativeDriver: Platform.OS !== 'web' }),
      Animated.timing(scaleAnim, { toValue: 0.97, duration: 220, useNativeDriver: Platform.OS !== 'web' }),
    ]).start(() => onClose());
  }, [onClose]);
