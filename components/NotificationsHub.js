...
// [WAVE 100] Module-scope live store — bell modal mounts/unmounts on each
// open, so cache that survives unmount kills the loading flash. Per-account
// scoped via userScopedKey from services/cache. MMKV layer below survives
// cold starts too.
const _hubLiveStore = new Map();
const HUB_KEY = 'notifications_feed_hub_v1';
function _readHubLive() {
  try {
    const key = userScopedKey(HUB_KEY);
    const hit = _hubLiveStore.get(key);
    if (hit && Array.isArray(hit.items)) return hit.items;
  } catch {}
  return null;
}
function _writeHubLive(items) {
  try {
    const key = userScopedKey(HUB_KEY);
    _hubLiveStore.set(key, { items, ts: Date.now() });
  } catch {}
}
import {
...
export default function NotificationsHub({
  ...
  // [WAVE 100] Lazy init from live store → MMKV → []. Modal opens with
  // notifications already rendered if we've fetched recently.
  const [items, setItems] = useState(() => {
    const live = _readHubLive();
    if (live) return live;
    try {
      const cached = getCachedSync(HUB_KEY);
